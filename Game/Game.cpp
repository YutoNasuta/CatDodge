...
void Game::Initialize(HWND window, int width, int height)
{
    ...
    */
    ...
    // 入力マネージャを作成する
    m_inputManager = std::make_unique<NakashiLib::InputManager>(window);

    // コモンステートを作成する
    m_commonStates = std::make_unique<CommonStates>(device);

    // デバッグ文字列を作成する
    m_debugString = std::make_unique<NakashiLib::DebugString>(
        device,
        context,
    ...
    // リソースマネージャーを作成する
    m_resourceManager = std::make_unique<NakashiLib::ResourcesManager>();
